#!/usr/bin/env python3
"""
SYNC version of database initializer - FIXED for bcrypt and MissingGreenlet error
Run: python scripts/init_db_sync_fixed.py --action create --force
"""
...
# Bypass bcrypt issue by using a simple hash for development
# WARNING: Only use this for development/testing!
import hashlib

def simple_hash(password: str) -> str:
    """Simple hash for development - DO NOT USE IN PRODUCTION"""
    return hashlib.sha256(f"dev_salt_{password}".encode()).hexdigest()

# Try to use real bcrypt, fallback to simple hash
try:
    from src.auth.jwt import get_password_hash
    USE_REAL_BCRYPT = True
    print("Using bcrypt for password hashing")
except Exception as e:
    print(f"bcrypt not available, using simple hash for development: {e}")
    USE_REAL_BCRYPT = False
    get_password_hash = simple_hash
...
def seed_demo_data():
    """Seed demo data with short passwords to avoid bcrypt issues"""
    db = SessionLocal()
    ...
    try:
        ...
        logger.info("🌱 Seeding demo data with short passwords...")
        
        # 1. Admin - using short password
        admin = User(
        ...
        db.add(admin)
        db.flush()
        logger.info(f"✅ Created Admin: {admin.username} (password: Admin123)")
        
        # 2. Members - short passwords
        members_data = [
        ...
        db.flush()
        logger.info(f"✅ Created {len(members)} Members (password: Member123)")
        
        # 3. Regular Users - short passwords
        regular_users = []
        for i in range(1, 11):
            user = User(
                username=f"user_{i}",
        ...
        db.flush()
        logger.info(f"✅ Created {len(regular_users)} Regular Users (password: User123)")
        ...
        logger.info("="*60)
        logger.info("\n🔐 Demo Accounts (short passwords):")
        logger.info(f"   Admin:     admin_system / Admin123")
        logger.info(f"   Member:    member_nguyen / Member123")
        logger.info(f"   Member:    member_tran / Member123")
        logger.info(f"   Member:    member_lee / Member123")
        logger.info(f"   Member:    member_pham / Member123")
        logger.info(f"   Member:    member_hoang / Member123")
        logger.info(f"   Regular:   user_1 / User123")
        logger.info("="*60)
