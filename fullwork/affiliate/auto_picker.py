...
def get_headers(access_key):
    return {"Authorization": f"Token {access_key}", "Content-Type": "application/json"}
...
def score_campaign(c):
    ...
    # Da duoc duyet â€” chay duoc ngay
    ...
    if c.get("commission_type") == "percentage":
        if max_comm >= 10:
            score += 25
            reasons.append(f"+25 hoa hong cao {max_comm}%")
        elif max_comm >= 5:
            score += 15
            reasons.append(f"+15 hoa hong kha {max_comm}%")
        elif max_comm > 0:
            score += 5
            reasons.append(f"+5  hoa hong {max_comm}%")
    else:  # fixed
        if max_comm >= 100000:
            score += 20
            reasons.append(f"+20 fixed {max_comm:,.0f}d/don")
        elif max_comm >= 30000:
            score += 10
            reasons.append(f"+10 fixed {max_comm:,.0f}d/don")

    # gross_commission = tong hoa hong da chi tra thuc te (popularity)
    gross = float(c.get("gross_commission") or 0)
    if gross >= 10_000_000:
        score += 20
        reasons.append(f"+20 gross={gross/1e6:.1f}M (rat hot)")
    elif gross >= 1_000_000:
        score += 10
        reasons.append(f"+10 gross={gross/1e6:.1f}M (hot)")
    elif gross > 0:
        score += 5
        reasons.append(f"+5  gross={gross/1e3:.0f}K")
    ...
    if cookie_d >= 30:
        score += 10
        reasons.append(f"+10 cookie={cookie_d}ngay")
    elif cookie_d >= 7:
        score += 5
        reasons.append(f"+5  cookie={cookie_d}ngay")
    ...
    if end:
        try:
            days_left = (datetime.fromisoformat(end.replace("Z", "").split("T")[0]) - datetime.utcnow()).days
            if days_left < 7:
                score -= 15
                reasons.append(f"-15 sap het han ({days_left}ngay)")
        except Exception:
            pass
    ...
    while True:
        r = requests.get(
            ...
            params={
                "page": page,
                "page_size": 50,
                "sort_by": "max_commission",
                "sort_order": "desc",
            },
            timeout=15,
        )
        if r.status_code != 200:
            logger.error(f"API loi: {r.status_code} â€” {r.text[:100]}")
            break
        body = r.json()
...
def create_tracking_link(headers, campaign_id, url=None):
    ...
    if url:
        payload["urls"] = [url]
    r = requests.post(f"{AT_API}/product_link/create", headers=headers, json=payload, timeout=15)
    if r.status_code == 200:
...
def auto_pick_and_setup(access_key, top_n=8, auto_create_link=True):
    SEP = "=" * 60
    print(f"\n{SEP}")
    print("  AUTO CAMPAIGN PICKER v2 â€” SEN V3 / AccessTrade")
    print(SEP)
    ...
    for i, c in enumerate(top, 1):
        icon = "âœ…" if c["approval"] == "successful" else "â³"
    ...
    if auto_create_link:
        ...
        for c in top:
            ...
            if "aff_link" in link:
                ...
                print(f"  âœ… {c['merchant']}: {short}")
            else:
                print(f"  âŒ Loi: {link}")
            results.append(c)
